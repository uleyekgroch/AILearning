        ...
        if self.knowledge_graph:
            for concept in scene.concepts:
                try:
                    ...
                    # 使用实际存在的 KG API
                    if hasattr(kg, 'get_relations_of'):
                        rels = kg.get_relations_of(concept)
                        for rel in rels[:5]:
                            target_id = rel.target_id if hasattr(rel, 'target_id') else str(rel)
                            rel_type = rel.type if hasattr(rel, 'type') else '关联'
                            if target_id in scene.concepts:
                                # 因果类关系给更高置信度
                                conf = 0.7 if rel_type in ('CAUSES', '导致', '引起') else 0.5
                                chains.append(CausalChain(
                                    steps=[concept, rel_type, target_id],
                                    confidence=conf,
                                    evidence=[f'KG: {concept}--{rel_type}-->{target_id}'],
                                ))
                    elif hasattr(kg, 'get_outgoing'):
