...
class Learner:
    ...
    def __init__(self, config: LearnerConfig):
        ...
        self._sleep_consolidation = {
            'consolidation_interval': 50,  # 每50条巩固一次
        ...
        self._init_commonsense_kb()

    # ------------------------------------------------------------------
    # 常识知识库
    # ------------------------------------------------------------------

    def _init_commonsense_kb(self):
        """初始化常识知识库"""
        try:
            ...
            seed_file = os.path.join(project_root, 'data', 'commonsense_seed.json')

            if os.path.exists(seed_file):
                self.commonsense_kb.load_from_file(seed_file)
                stats = self.commonsense_kb.get_stats()
                print(f"[OK] 常识知识库初始化: {stats['total_facts']}条事实")
            else:
                print(f"[WARN] 常识种子文件不存在: {seed_file}")
        except Exception as e:
            print(f"[ERROR] 常识知识库初始化失败: {e}")
            self.commonsense_kb = None

    def query_commonsense(self, question: str, top_k: int = 3) -> list:
        """查询常识知识库
    ...
    def add_commonsense_fact(self, statement: str, fact_type: str, confidence: float = 0.8):
        """添加常识事实
        ...
        """
        if self.commonsense_kb:
            from src.knowledge.commonsense import CommonsenseFact
    ...
    def _improve_causal_rules(self) -> Dict:
        ...
        if self._registry.has('causal_dag'):
            dag = self._safe_registry_get('causal_dag')
            # 分析现有规则，生成新规则
            new_rules = []
    ...
    def _improve_concept_formation(self) -> Dict:
        ...
        if self._registry.has('concept_formation'):
            cf = self._safe_registry_get('concept_formation')
            # 分析现有概念，生成新概念
            new_concepts = []
    ...
    def learn_from_experience(self, obs: torch.Tensor, action,
        ...
        if self._registry.has('plasticity'):
            p = self._safe_registry_get('plasticity').get_plasticity(self._total_steps)
            grad = grad * p
        ...
        # ===== 新增：世界模拟器学习 =====
        try:
            simulator = self._safe_registry_get('world_simulator')
        ...
        try:
            concept_formation = self._safe_registry_get('concept_formation')
            # 使用实际观察作为概念表示
    ...
    def _register_perceptual_concept(self, concept_data: Dict) -> bool:
        ...
        # 获取概念空间
        cs = self._safe_registry_get('concept_space') if self._registry.has('concept_space') else None
        if not cs:
            return False
    ...
    def learn_from_text(self, text: str, source: str = "text") -> Dict:
        ...
        # 1b. 测试时训练：已禁用（非人类学习机制）
        # 原TestTimeTraining被移除 - 人类学习无需持续训练
        pass
        ...
        if self.config.statistical_learning_enabled:
            stat_learner = self._safe_registry_get('statistical_learner')
        ...
        # 23. GHL全局调制（暂时禁用 - O(n²)瓶颈）
        # TODO: 重新设计为基于概念空间的关系权重更新
        pass
        ...
        for i in range(len(entities) - 1):
            ...
            self._stdp_system['connections'][pair] = min(
                self._stdp_system['connections'][pair], 1.0
            )
        ...
        # 建立索引
        for e in entities:
            if e not in self._hippocampal_memory['index']:
                self._hippocampal_memory['index'][e] = []
            self._hippocampal_memory['index'][e].append(episode['timestamp'])

        self._hippocampal_memory['episodes'].append(episode)

        # 海马容量管理（随机遗忘）
        if len(self._hippocampal_memory['episodes']) > self._hippocampal_memory['capacity']:
            import random
            if random.random() < 0.1:
                idx = random.randint(0, len(self._hippocampal_memory['episodes']) - 1)
                self._hippocampal_memory['episodes'][idx] = self._hippocampal_memory['episodes'][-1]
                self._hippocampal_memory['episodes'].pop()

        # ===== 人类式学习系统：睡眠巩固 =====
        # 定期执行睡眠巩固（每N条）
        self._sleep_consolidation['last_consolidation'] += 1
        if self._sleep_consolidation['last_consolidation'] >= self._sleep_consolidation['consolidation_interval']:
            self._sleep_consolidation['last_consolidation'] = 0
            self._sleep_consolidation['consolidation_count'] += 1

            # 将最近的记忆转移到"皮层"（概念空间）
            recent = self._hippocampal_memory['episodes'][-100:]
            cs = self._safe_registry_get('concept_space')
            if cs and recent:
                for ep in recent:
                    # 提取模式
                    for e in ep.get('entities', [])[:5]:
                        if e not in cs.concepts:
                            # 创建概念
                            cs.concepts[e] = type('Concept', (), {
                                'id': e,
                                'vector': self._encode_text(e),
    ...
    def _verify_learned_knowledge(self, text: str, entities: List[str],
        ...
        for item in triples[:3]:
            ...
            # 轻量级验证：直接查KG而非调用完整think()
            answer = ""
            kg = self.knowledge
            if kg and hasattr(kg, "get_relations_of"):
                try:
                    answer = " ".join(r.target_id for r in kg.get_relations_of(subject)[:2] if hasattr(r, "target_id"))
                except Exception:
                    pass
    ...
    def _encode_text(self, text: str, train: bool = False) -> torch.Tensor:
        ...
        if not words:
            result = torch.zeros(self._perception_dim).to(device)
        else:
            # 获取词向量
            vectors = []
            for word in words:
    ...
    def _train_embedding(self, text: str, entities: List[str]):
        ...
        ct = self._safe_registry_get('contrastive_trainer')
    ...
    def _safe_registry_get(self, name: str):
        """安全获取注册模块，未注册返回None"""
    ...
    def _extract_entities_from_repr(self, text: str, repr: torch.Tensor) -> List[str]:
        ...
        # === 主通道：统计学习涌现概念（不重复observe，learn_from_text已调用）===
        stat = self._safe_registry_get('statistical_learner')
        if stat:
            # observe已在learn_from_text Step 1c调用，这里只获取结果
            if hasattr(stat, 'get_emergent_concepts'):
                emergent = stat.get_emergent_concepts(min_freq=1, filter_boundary=True)
                # get_emergent_concepts 返回 List[Tuple[str, float]]
                entities = [c for c, conf in emergent if len(c) >= 2]

        # === 补充通道：感知-预测循环的误差概念 ===
        pll = self._safe_registry_get('perception_learning_loop')
        if pll:
            try:
                # 先执行感知循环，让误差概念从预测误差中涌现
                if hasattr(pll, 'perceive_and_learn'):
                    result = pll.perceive_and_learn(source_text)
                    if isinstance(result, dict):
                        for ec in result.get('new_concepts', []):
                            label = ec.get('label', '') if isinstance(ec, dict) else str(ec)
    ...
    def _extract_relations_from_repr(self, text: str, entities: List[str], repr: torch.Tensor) -> List[Tuple[str, str, str]]:
        """从文本和表示中提取关系 - verb-pivot方法

        三层策略:
        1. Primary: verb-pivot — 在文本中找 entity1...verb...entity2 模式
        2. Secondary: concept_space Hebbian关联
        3. Fallback: 简单共现
        """
        import re
        relations = []
        ...
        # === Primary: Verb-pivot extraction ===
        # 中文动词模式 - 开放集合，不限于固定列表
        # 匹配: entity1 + 可选修饰 + 动词短语 + entity2
        verb_pattern = re.compile(
            r'([一-鿿]+?)[\s，,]*([一-鿿]{1,4}(?:是|为|有|能|会|可以|属于|包含|组成|构成|'
            r'导致|引起|促进|抑制|影响|决定|产生|形成|依赖|需要|使用|利用|'
            r'转化|变成|来自|源于|通过|经过|进入|离开|连接|分离|'
            r'吃|喝|住|飞|跑|游|长|开|关|升|降|'
            r'[一-鿿]{1,2})了?着?过?)\s*([一-鿿]+)'
        )
        ...
        for sent in text_sentences:
            if not sent.strip():
                continue
            for m in verb_pattern.finditer(sent):
                subj, verb, obj = m.group(1), m.group(2), m.group(3)
                # Check if subj/obj overlap with known entities
                subj_match = next((e for e in entity_set if e in subj or subj in e), None)
                obj_match = next((e for e in entity_set if e in obj or obj in e), None)
                if subj_match and obj_match and subj_match != obj_match:
                    # Clean verb - extract core verb
                    core_verb = re.sub(r'^(不|很|也|都|正在|已经|将要)', '', verb)
                    if core_verb:
                        relations.append((subj_match, core_verb, obj_match, 0.8))

        # Also try simpler patterns: entity1 是/为 entity2 的 X
        # 用字符串搜索代替每对实体 compile 正则
        copulas = ['是', '为', '叫', '叫做', '即']
        for e1 in entities:
            for e2 in entities:
                if e1 == e2:
                ...
                # Direct "A是B" pattern
                for cop in copulas:
                    if e1 + cop in text and text.find(e1) < text.find(e2):
                        relations.append((e1, '是', e2, 0.9))
                        break
                # "A的B" possession pattern
                if e1 + '的' + e2 in text:
                    relations.append((e1, '具有', e2, 0.7))

        # === Secondary: Concept space Hebbian relations ===
        if len(relations) < 2:
            try:
                cs = self._safe_registry_get('concept_space')
                if cs:
                    for i, e1 in enumerate(entities[:8]):
                        for e2 in entities[i+1:8]:
                            rel = cs.get_relation(e1, e2) if hasattr(cs, 'get_relation') else None
                            if rel:
                                relations.append((e1, rel.get('type', '关联'), e2, rel.get('weight', 0.5)))
                            elif hasattr(cs, 'concepts'):
                                # Check Hebbian co-activation
                                c1 = cs.concepts.get(e1)
                                c2 = cs.concepts.get(e2)
                                if c1 is not None and c2 is not None:
                                    if hasattr(c1, 'connections') and e2 in c1.connections:
                                        w = c1.connections[e2]
                                        if w > 0.1:
                                            relations.append((e1, '关联', e2, min(w, 0.6)))
            except Exception:
                pass

        # === Fallback: Representation similarity co-occurrence ===
        if len(relations) < 1 and len(entities) >= 2:
            # Use repr similarity between entity pairs as co-occurrence signal
            for i, e1 in enumerate(entities[:5]):
                for e2 in entities[i+1:5]:
                    # Simple co-occurrence: both in same sentence
                    for sent in text_sentences:
                        if e1 in sent and e2 in sent:
                            relations.append((e1, '共现', e2, 0.3))
                            break
        ...
        unique = []
        for r in relations:
            key = (r[0], r[2]) if r[0] < r[2] else (r[2], r[0])
            if key not in seen:
                seen.add(key)
                unique.append(r)
    ...
    def think(self, question: str) -> str:
        ...
        # ===== 新路径：基于STDP连接的推理 =====
        # 提取问题中的实体
        q_entities = re.findall(r'[一-鿿]{2,6}', question)
        if q_entities:
            # 通过STDP连接找相关概念
            stdp_results = []
            for entity in q_entities[:3]:
                # 查找以该实体为pre的所有连接
                for (pre, post), weight in self._stdp_system['connections'].items():
                    if pre == entity and weight > 0.2:
                        stdp_results.append((post, weight))
                    elif post == entity and weight > 0.2:
                        stdp_results.append((pre, weight))

            if stdp_results:
                # 按权重排序
                stdp_results.sort(key=lambda x: -x[1])
                top_concepts = [c for c, w in stdp_results[:5] if c not in q_entities]
                if top_concepts:
                    return f"{q_entities[0]}与{'、'.join(top_concepts[:3])}有较强的时序关联"

        # ===== 海马记忆检索 =====
        if q_entities:
            for entity in q_entities[:2]:
                if entity in self._hippocampal_memory['index']:
                    indices = self._hippocampal_memory['index'][entity]
                    if indices:
                        # 获取最近的记忆
                        recent_idx = indices[-1]
                        if recent_idx < len(self._hippocampal_memory['episodes']):
                            episode = self._hippocampal_memory['episodes'][recent_idx]
                            context = episode.get('context', '')
                            if context and len(context) > 5:
                                return f"根据记忆，{context[:50]}"

        # Path 0: SimulationReasoning as PRIMARY
        try:
            sr = self.simulation_reasoning  # property, 不走registry
            if sr:
                cs = self._safe_registry_get("concept_space")
                context_concepts = []
                ...
                if context_concepts:
                    ...
                    if sr_result and sr_result.confidence >= 0.2:
                        # 先尝试 compose 自然表达
                        try:
        ...
        # Path 0.5: LanguageAcquisition compose for expression
        try:
            la = self.language_acquisition  # property, 不走registry
            if la and hasattr(la, "compose"):
                cs = self._safe_registry_get("concept_space")
                if cs and hasattr(cs, "activate"):
                    act = cs.activate(question, top_k=3, spread_depth=1)
                    if act and act[0].activation > 0.15:
                        concept_ids = [a.concept_id for a in act[:5]]
                        composed = la.compose(concept_ids, goal=question)
                        if composed and len(composed) > 5:
                            return composed
        except Exception:
            pass
        ...
        try:
            ...
            if cs and len(cs.concepts) >= 3:
                ...
                if activated and activated[0].activation > 0.25:
            ...
            if cs and hasattr(cs, "analogy") and hasattr(cs, "activate"):
                ...
                if q_activated and len(q_activated) >= 2:
                    ...
                    for ac in q_activated[:2]:
                        ...
                        if analogies:
                            analog_id, score = analogies[0]
                            # 用类比概念的关系来推断答案
                            analog_rels = cs.relations.get(analog_id, {})
                            if analog_rels:
                                best_rel = max(analog_rels.items(), key=lambda x: x[1])
                                answer = f"{c}可能与{best_rel[0]}有类似关系（类比自{analog_id}，相似度{score:.1%}）"
                                if len(answer) > 5:
                                    return answer
                    if answer and len(answer) > 5:
                        return answer
        ...
        for ac in activated[:15]:
            ...
            if kg and hasattr(kg, 'get_relations_of'):
                ...
                for rel in rels[:3]:
                    ...
                    # 根据关系类型构建自然句式
                    la = self._safe_registry_get('language_acquisition')
                    if la and hasattr(la, 'compose'):
                        sentence = la.compose([cid, rel_type, target], goal=question)
                    else:
    ...
    def _synthesize_multihop(self, cs, activated: list, question: str) -> str:
        ...
        # 只导入通过质量检查的概念
        verified_set = set()
        stat_learner = self._safe_registry_get('statistical_learner') if self._registry.has('statistical_learner') else None
        kg = self.knowledge
    ...
    def knowledge(self) -> KnowledgeGraph:
        return self._safe_registry_get('knowledge')

    @property
    def lang_bridge(self) -> LanguageGraphBridge:
        return self._safe_registry_get('lang_bridge')

    @property
    def reasoning(self):
        return self._safe_registry_get('reasoning')

    @property
    def metacognition(self):
        return self._safe_registry_get('metacognition')

    @property
    def hypothesis_engine(self):
        return self._safe_registry_get('hypothesis')

    @property
    def goals(self):
        return self._safe_registry_get('goals')

    @property
    def transfer(self):
        return self._safe_registry_get('transfer')

    @property
    def observation_learner(self):
        return self._safe_registry_get('observation')

    @property
    def teaching_module(self):
        return self._safe_registry_get('teaching')

    @property
    def collaboration(self):
        return self._safe_registry_get('collaboration')

    @property
    def active_inference(self):
        return self._safe_registry_get('active_inference')

    @property
    def fep_predictor(self):
        return self._safe_registry_get('fep_predictor')

    @property
    def fep_belief(self):
        return self._safe_registry_get('fep_belief')

    @property
    def plasticity(self):
        return self._safe_registry_get('plasticity')

    @property
    def consolidation_engine(self):
        return self._safe_registry_get('consolidation')

    @property
    def motivation(self):
        return self._safe_registry_get('motivation')

    @property
    def metaphor_tracker(self):
        return self._safe_registry_get('metaphor')

    @property
    def theory_of_mind(self):
        return self._safe_registry_get('theory_of_mind')

    @property
    def causal(self):
        return self._safe_registry_get('causal')

    @property
    def counterfactual(self):
        return self._safe_registry_get('counterfactual')

    @property
    def tool_use(self):
        return self._safe_registry_get('tool_use')

    @property
    def inner_speech(self):
        return self._safe_registry_get('inner_speech')

    @property
    def narrative(self):
        return self._safe_registry_get('narrative')

    @property
    def crossmodal(self):
        return self._safe_registry_get('crossmodal')

    @property
    def attention(self):
        return self._safe_registry_get('attention')

    @property
    def adversarial(self):
        return self._safe_registry_get('adversarial')

    @property
    def cooperative_planner(self):
        return self._safe_registry_get('cooperative_planner')

    @property
    def questioning(self):
        return self._safe_registry_get('questioning')

    @property
    def nonstationary(self):
        return self._safe_registry_get('nonstationary')

    @property
    def dual_memory(self):
        return self._safe_registry_get('dual_memory')

    @property
    def continuous_concepts(self):
        return self._safe_registry_get('continuous_concepts')

    # ===== 新增层属性 =====
    @property
    def causal_dag(self):
        return self._safe_registry_get('causal_dag')

    @property
    def concept_formation(self):
        return self._safe_registry_get('concept_formation')

    @property
    def world_simulator(self):
        return self._safe_registry_get('world_simulator')

    @property
    def numerical_understanding(self):
        return self._safe_registry_get('numerical')

    @property
    def analogical_reasoning(self):
        return self._safe_registry_get('analogical')

    @property
    def metacognition_enhanced(self):
        return self._safe_registry_get('metacognition_enhanced')
    ...
    def get_stats(self) -> Dict:
        ...
        if self._registry.has('goals'):
            stats['active_goals'] = len(self._safe_registry_get('goals').goals)
        if self._registry.has('observation'):
            stats['observations_total'] = self._safe_registry_get('observation').get_observation_count()
        if self._registry.has('transfer'):
            stats['transfers_completed'] = len(self._safe_registry_get('transfer').transfer_history)
        if self._registry.has('plasticity'):
            stats['plasticity'] = self._safe_registry_get('plasticity').get_plasticity(self._total_steps)
        if self._registry.has('consolidation'):
            stats['consolidated_traces'] = len(self._safe_registry_get('consolidation').traces)
        if self._registry.has('metaphor'):
            m = self._safe_registry_get('metaphor').detect_metaphors()
        ...
        try:
            ...
            pass
        try:
            ws = self._safe_registry_get('world_simulator')
            stats['simulations'] = ws.stats.get('simulations', 0)
        except (KeyError, Exception):
            pass
        try:
            num = self._safe_registry_get('numerical')
            stats['numerical_facts'] = num.stats.get('facts_stored', 0)
        except (KeyError, Exception):
            pass
        try:
            ana = self._safe_registry_get('analogical')
    ...
    def save(self, path: str) -> None:
        ...
        for reg_name, state_key in _MODULE_STATE_KEYS.items():
            if not self._registry.has(reg_name):
                continue
            mod = self._safe_registry_get(reg_name)
        ...
        if self._registry.has('active_inference'):
            state['fep_predictor_state'] = self._safe_registry_get('fep_predictor').state_dict()
            state['fep_belief_state'] = self._safe_registry_get('fep_belief').save_state()
    ...
    def load(self, path: str) -> None:
        ...
        for reg_name, (state_key, module_path, class_name) in _load_map_kg.items():
            ...
            instance = cls(self._safe_registry_get('knowledge'))
        ...
        if 'goals_state' in state:
            from src.goals.manager import GoalManager
            g = GoalManager(self._safe_registry_get('knowledge'), self._safe_registry_get('metacognition'))
