...
## Phase 50: 跨语言迁移（不同环境的语言互译）
**Status:** complete ✅

不同环境中的 Agent 发展出不同语言后，能否互相翻译？

### 文件变更
| 文件 | 变更 |
|------|------|
| language_translator.py | 新建 — CrossLingualAgent + CrossLingualSociety |
| experiment_cross_lingual.py | 新建 — 4 个实验 |

### 实验结果

**实验 1：跨环境基线（热带 vs 极地）**
- 隔离后群体内成功率：~95%
- 跨环境基线：96.5%（接触前）→ 95.5%（接触后）
- 符号重叠：0.700 → 0.850
- 群体 B 独有符号：bright, dark, dim（极地特有亮度属性）
...
- 500 Agent，5 个不同环境
- 符号重叠：94-100%（几乎完全共享）
- 语言相似度：0.33-0.52（符号含义仍在分化）

### 关键发现
1. **符号重叠 ≠ 语义相同**：5 群体共享 94-100% 符号，但语言相似度仅 0.33-0.52。同形异义词现象。
2. **双语者 > 桥接 Agent**：桥接引入翻译链噪声（82.8%），双语者直接学习两种语言（97.9%）
3. **环境差异影响有限**：属性空间重叠大，即使高度不同环境跨环境成功率也在 93-97%
4. **符号快速趋同**：跨区域接触后符号重叠从 0.10-0.70 快速升至 0.53-1.00

---

## Phase 51: 自主目标设定（Agent 自己决定学什么）
**Status:** complete ✅

Agent 自主评估知识状态，识别薄弱维度，针对性练习。

### 文件变更
| 文件 | 变更 |
|------|------|
| self_directed_learning.py | 新建 — KnowledgeAssessor + GoalSelector + SelfDirectedLearner |
| experiment_self_directed.py | 新建 — 3 个实验 |
| language_emergence.py | 修复 — 添加 6 个缺失符号集及其 _symbol_category 映射 |

### 实验结果

**实验 1：自主 vs 随机 vs 均匀（300 轮 x 5 对）**
| 策略 | 成功率 | 词汇量 | 维度覆盖 |
|------|--------|--------|---------|
| 自主学习 | 74.7% | 32.4 | 10/10 |
| 随机学习 | 94.3% | 40.0 | 4/10 |
| 均匀学习 | 93.3% | 40.0 | 4/10 |

**实验 2：知识差距恢复（200 轮）**
- 偏置训练（3 维度 100 轮）→ 恢复训练（100 轮）
- 自主学习：+7 维度（3→10），成功率 62-84%
- 随机学习：+7 维度（3→10），成功率 91-95%

**实验 3：目标适应性（300 轮）**
- Round 50: 聚焦 pattern+size（最弱维度）
- Round 300: 继续聚焦 pattern+size（仍未完全掌握）
- 最终 8/10 维度 mastered，2/10 learning

### 关键发现
1. **维度覆盖 10/10 vs 4/10**：自主学习覆盖全部维度，随机/均匀只覆盖 4 个
2. **成功率低 = 刻意练习**：自主 Agent 练习不会的，随机 Agent 重复已掌握的
3. **目标转移自然发生**：已掌握维度自动退出目标列表
4. **符号集修复**：_symbol_category 缺失 6 个属性类别（texture/weight/temperature/brightness/pattern/origin）

---

## Phase 52: 课程涌现（歧义度驱动的难度阶梯）
**Status:** complete ✅

研究场景歧义度（非复杂度）对学习的影响，验证课程排序是否有效。

### 核心转折

v1 用"复杂度"（物体数+属性数）衡量难度，结果"困难"场景反而成功率最高（100%）——因为更多属性 = 更多区分信息 = 更容易。v2 改为用**歧义度**（共享属性数量）衡量难度。

### 文件变更
| 文件 | 变更 |
|------|------|
| curriculum_learning.py | 新建 — 歧义度控制场景生成 + 渐进课程 + 自主节奏 |
| experiment_curriculum.py | 新建 — 4 个实验 |

### 歧义等级设计

| 等级 | 物体数 | 总属性 | 共享属性 | 有效区分维度 |
|------|--------|--------|---------|------------|
| 1 | 3 | 4 | 0 | 4 |
| 2 | 4 | 4 | 1 | 3 |
| 3 | 5 | 4 | 1 | 3 |
...
**实验 1：5 种策略对比（500 轮 x 5 对）**

| 策略 | 成功率 | 词汇量 | 平均歧义度 |
|------|--------|--------|-----------|
| easy（等级 1） | 99.9% | 22 | 0.183 |
| progressive | 78.6% | 22 | 0.528 |
| random | 78.4% | 22 | 0.530 |
| self_paced | 67.8% | 21 | 0.646 |
| hard（等级 7） | 44.8% | 13 | 0.734 |

**实验 2：自主节奏轨迹（500 轮 x 3 次）**
- 平均成功率：68.4%
- 平均最终等级：4.9（从 1.0 开始，快速升至 5.0 并稳定）

**实验 3：歧义度对成功率的影响（每等级 200 轮）**
...
### 关键发现
1. **歧义度是真正的难度因子**：有效区分维度从 4 降到 1，成功率从 100% 降到 47%
2. **"先难后易"优于"先易后难"**：81.6% vs 79.7%，与课程学习假设相反
3. **课程排序影响极小**（78.6%-81.6%，仅 3% 差距）
4. **原因**：Speaker.describe() 是无状态计算——每次根据场景特征重新计算，不利用历史经验。被动追踪的词汇统计不反馈到描述策略中
5. **1 维度瓶颈**：当只有 1 个属性区分物体时，6-10 个物体中只有 4-5 个唯一，导致近半数场景无法区分
6. **自主节奏过冲**：从等级 1 快速升至等级 5，卡在高歧义区域，拉低平均成功率

### 更新的文件
- mvl/curriculum_learning.py（新建：歧义度控制 + 渐进课程）
- mvl/experiment_curriculum.py（新建：4 个实验）
- mvl/curriculum_results.json（新建：实验结果）
- theory_framework.md（添加 5.56 课程涌现）
- README.md（添加 Phase 52）
